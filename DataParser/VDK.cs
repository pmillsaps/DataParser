...
namespace DataParser
{
    public class VDK
    {
        ...
        private static List<PartRevision> partRevisions = new List<PartRevision>();
        private static List<PartRevision> partRevisionApprovals = new List<PartRevision>();
        private static List<PartMaster> currentParts { get; set; }
        private static List<DMT_INV_Part_Combined> newParts { get; set; }
        ...
        private static List<UOM> UOMLookup { get; set; }
        ...
        private const string CompanyID = "VDK";
        ...
        public static void Process()
        {
            ...
            UOMLookup = BuildUOMLookup();
            dmt_BOM = BuildBOM();
            string currentPart = "";
            List<String> UniqueParts = new List<string>();
            newParts = new List<DMT_INV_Part_Combined>();
            partRevisions = new List<PartRevision>();
            partRevisionApprovals = new List<PartRevision>();
            var tmpBOM = dmt_BOM.ToList();

            foreach (var item in tmpBOM.OrderBy(x => x.PartNum).ThenBy(x => x.MtlPartNum))
            {
                if (currentPart != item.PartNum)
                {
                    Console.WriteLine("Starting Item: {0}", item.PartNum);
                    currentPart = item.PartNum;
                    if (currentParts.FirstOrDefault(x => x.PartNum == item.PartNum) == null)
                    {
                        MissingParts += String.Format("BOM Top Level Missing: {0}", item.PartNum) + Environment.NewLine;
                        if (!UniqueParts.Contains(item.PartNum))
                        {
                            UniqueParts.Add(item.PartNum);
                        }
                    }
                    //Added newpart anytime since we want to make sure they exist
                    if (!newParts.Any(x => x.PartNum == item.PartNum))
                    {
                        var newpart = new DMT_INV_Part_Combined
                        {
                            Company = "VDK",
                            PartNum = item.PartNum,
                            PartDescription = "Missing Part - Top Level",
                            BuyToOrder = "False",
                            ClassID = "1",
                            ProdCode = "1",
                            CostMethod = "A",
                            ...
                            TypeCode = "M",
                            UnitPrice = 0,
                            UOMClassID = UOMLookup.First(x => x.UOMCode == item.UOMCode).UOMClassId,
                            PartPlant__GenerateSugg = "True",
                            PartPlant__Plant = "MfgSys",
                            PartPlant__ProcessMRP = "True",
                            PartPlant__QtyBearing = "True",
                            PartWhse__WarehouseCode = "1",
                        };

                        if (currentParts.FirstOrDefault(x => x.PartNum == item.PartNum) != null)
                        {
                            var prt = currentParts.FirstOrDefault(x => x.PartNum == item.PartNum);
                            newpart.BuyToOrder = prt.BuyToOrder.ToString();
                            newpart.ClassID = prt.ClassID;
                            newpart.CostMethod = prt.CostMethod;
                            newpart.IUM = prt.IUM;
                            newpart.UOMClassID = prt.UOMClassID;
                            newpart.PartDescription = CleanString(prt.PartDescription);
                            newpart.PhantomBOM = prt.PhantomBOM.ToString();
                            newpart.ProdCode = prt.ProdCode;
                            newpart.PUM = prt.PUM;
                            newpart.TypeCode = prt.TypeCode;
                        }

                        if (dmt_BOM.Any(x => x.PartNum == newpart.PartNum))
                        {
                            newpart.TypeCode = "M";
                            newpart.PartRev__RevisionNum = "NEW";
                            newpart.PartRev__RevShortDesc = "New";
                            newpart.PartRev__RevDescription = "New";
                            newpart.PartRev__EffectiveDate = "2018-01-01";
                        }

                        newParts.Add(newpart);

                        partRevisionApprovals.Add(new PartRevision
                        {
                            Company = "VDK",
                            Plant = "MfgSys",
                            PartNum = currentPart,
                    ...
                    // Create PartRev Entry for the top level BOM

                    if (!partRevisions.Any(x => x.PartNum == currentPart))
                    {
                        partRevisions.Add(new PartRevision
                        {
                            Company = "VDK",
                            Plant = "MfgSys",
                            PartNum = currentPart,
                ...
                if (currentParts.FirstOrDefault(x => x.PartNum == item.MtlPartNum) == null)
                {
                    MissingParts += String.Format("BOM Lower Level Missing: {0} - {1}", item.PartNum, item.MtlPartNum) + Environment.NewLine;
                    if (!UniqueParts.Contains(item.MtlPartNum))
                    {
                        UniqueParts.Add(item.MtlPartNum);
                    }
                }

                var partcheck = item.MtlPartNum;
                //string levelMsg = "Lower Level";

                // Add newpart for all parts
                if (!newParts.Any(x => x.PartNum == partcheck))
                {
                    var newpart = new DMT_INV_Part_Combined
                    {
                        Company = "VDK",
                        PartNum = partcheck,
                        PartDescription = "Missing Part - Lower Level",
                        BuyToOrder = "False",
                        ClassID = "1",
                        ProdCode = "1",
                        CostMethod = "A",
                    ...
                    if (currentParts.FirstOrDefault(x => x.PartNum == newpart.PartNum) != null)
                    {
                        var prt = currentParts.FirstOrDefault(x => x.PartNum == item.MtlPartNum);
                        newpart.BuyToOrder = prt.BuyToOrder.ToString();
                        newpart.ClassID = prt.ClassID;
                        newpart.CostMethod = prt.CostMethod;
                        newpart.IUM = prt.IUM;
            ...
            MissingParts = "Missing Parts:" + Environment.NewLine + string.Join(Environment.NewLine, UniqueParts.OrderBy(x => x)) + Environment.NewLine + Environment.NewLine + MissingParts;
            ...
            Console.WriteLine("Writing Data Files...");

            WriteDMTBOM("DMT-98-BillOfMaterials-Generated.csv", dmt_BOM);
            WritePartCombined("DMT-93-Parts-Missing.csv", newParts);
            WritePartRevisions("DMT-96-PartRevisions.csv", partRevisions);
            WritePartRevisions("DMT-96-PartRevisionApprovals.csv", partRevisionApprovals);
        ...
        private static List<UOM> BuildUOMLookup()
        {
            var uomList = new List<UOM>();

            uomList.Add(new UOM { UOMCode = "EA", UOMClassId = "COUNT" });
            uomList.Add(new UOM { UOMCode = "PR", UOMClassId = "COUNT" });
            uomList.Add(new UOM { UOMCode = "FT", UOMClassId = "LENGTH" });
            uomList.Add(new UOM { UOMCode = "M", UOMClassId = "LENGTH" });
            uomList.Add(new UOM { UOMCode = "KM", UOMClassId = "LENGTH" });
            uomList.Add(new UOM { UOMCode = "LT", UOMClassId = "VOLUME" });
            uomList.Add(new UOM { UOMCode = "KG", UOMClassId = "WEIGHT" });
            uomList.Add(new UOM { UOMCode = "HR", UOMClassId = "TIME" });
            uomList.Add(new UOM { UOMCode = "M2", UOMClassId = "AREA" });

            return uomList;
        }

        private static List<DMT_BillOfMaterial> BuildBOM()
        {
            List<DMT_BillOfMaterial> temp = new List<DMT_BillOfMaterial>();

            string BOMFolder = @"C:\Dropbox\VDK\Dung-Phuong\3BOM_CSV_files";
            var files = Directory.EnumerateFiles(BOMFolder);
            foreach (var item in files)
            {
                Console.WriteLine(String.Format("Importing File {0}", item));
                var tmp = ImportBOM(item);
                temp.AddRange(tmp);
                //bool mismatch = false;
                //var distinctParts = tmp.DistinctBy(x => x.PartNum).Select(x => x.PartNum).ToList();

                //foreach (var bomitem in distinctParts)
                //{
                //    if (temp.Any(x => x.PartNum == bomitem))
                //    {
                //        var tmp1 = tmp.Where(x => x.PartNum == bomitem).ToList();
                //        var tmp2 = temp.Where(x => x.PartNum == bomitem).ToList();
                //        if (tmp1.Count() != tmp2.Count())
                //        {
                //            ErrorList += String.Format("Item Count Mismatch: {0} | {1} | {2}", tmp1.Count(), tmp2.Count(), item);
                //            mismatch = true;
                //        }

                //        foreach (var inside1 in tmp1)
                //        {
                //            var match = tmp2.FirstOrDefault(x => x.MtlPartNum == inside1.MtlPartNum);
                //            if (match == null)
                //            {
                //                ErrorList += String.Format("New Item not in Old BOM List: {0} | {1} | {2}", inside1.PartNum, inside1.MtlPartNum, inside1.MtlSeq);
                //                mismatch = true;
                //            }
                //        }

                //        foreach (var inside1 in tmp2)
                //        {
                //            var match = tmp1.FirstOrDefault(x => x.MtlPartNum == inside1.MtlPartNum);
                //            if (match == null)
                //            {
                //                ErrorList += String.Format("Old Item not in New BOM List: {0} | {1} | {2}", inside1.PartNum, inside1.MtlPartNum, inside1.MtlSeq);
                //                mismatch = true;
                //            }
                //        }
                //    }
            ...
            return CleanUpBOM(temp);
        }

        private static List<DMT_BillOfMaterial> CleanUpBOM(List<DMT_BillOfMaterial> temp)
        {
            Console.WriteLine("Cleaning BOM's...");
            var cleanBOM = new List<DMT_BillOfMaterial>();
            foreach (var item in temp.Where(x => x.MtlPartNum != "TEXT" && x.MtlPartNum != "").OrderBy(x => x.PartNum))
            {
                if (cleanBOM.FirstOrDefault(x => x.PartNum == item.PartNum && x.MtlPartNum == item.MtlPartNum) == null)
                {
                    if (item.PartNum.Contains(" ") || item.MtlPartNum.Contains(" "))
                    {
                        item.PartNum = item.PartNum.Replace(" ", "");
                        item.MtlPartNum = item.MtlPartNum.Replace(" ", "");
                    }

                    item.PartNum = CleanString(item.PartNum);
                    item.MtlPartNum = CleanString(item.MtlPartNum);
                    item.QtyPer = item.QtyPer;

                    cleanBOM.Add(item);
                    Console.WriteLine("Cleaning BOM: Added Item {0} || {1}", item.PartNum, item.MtlPartNum);
                }
            }

            return ResequenceBOM(cleanBOM);
        }
        ...
        private static List<DMT_BillOfMaterial> ResequenceBOM(List<DMT_BillOfMaterial> cleanBOM)
        {
            ...
            var newList = cleanBOM.OrderByDescending(x => x.PartNum).ThenBy(x => x.MtlPartNum).ToList();
            int sequence = 10;
            string holdPart = "";
            foreach (var item in newList)
            {
                if (item.PartNum != holdPart)
                {
                    sequence = 10;
                    holdPart = item.PartNum;
                    Console.WriteLine("ReSequencing {0}", holdPart);
                }
        ...
        private static List<DMT_BillOfMaterial> ImportBOM(string filename)
        {
            var tempfile = FixHeader(filename);
            //var filePath = Path.Combine(Environment.CurrentDirectory, "Data", CompanyID, filename);
            StreamReader textreader = new StreamReader(tempfile);
            ...
            var list = csv.GetRecords<BOM>()
                .OrderBy(x => x.PartNum).ThenBy(x => x.MtlSeq)
                .ToList();
            csv.Dispose();
            textreader.Close();
            List<DMT_BillOfMaterial> ret = mapper.Map<List<BOM>, List<DMT_BillOfMaterial>>(list);

            foreach (var item in ret.ToList())
            {
                if (currentParts.FirstOrDefault(x => x.PartNum == item.MtlPartNum) != null)
                {
                    item.UOMCode = currentParts.FirstOrDefault(x => x.PartNum == item.MtlPartNum).IUM;
                }
                else
                {
                    item.UOMCode = "EA";
        ...
        private static string FixHeader(string filename)
        {
            ...
            File.WriteAllLines(outFile, li);

            return outFile;
        }

        private static string FixOutputHeader(string filename)
        {
            ...
            //StreamWriter streamWriter = new StreamWriter(fileName);
            var li = File.ReadAllLines(tempFile);
            li[0] = li[0].Replace("_", "#").Replace("#c,", "_c,");
            li[0] = li[0].Replace("#c,", "_c,");

            File.WriteAllLines(filename, li);
            File.Delete(tempFile);
            return filename;
        }
        ...
        {
            var filePath = Path.Combine(Environment.CurrentDirectory, "Data", CompanyID, "DMT-93-Parts.csv");
            StreamReader textreader = new StreamReader(filePath);
            var csv = new CsvReader(textreader);
            // Turn off.
            csv.Configuration.IgnoreBlankLines = true;
        ...
        private static void WritePartCombined(string fileName, List<DMT_INV_Part_Combined> data)
        {
            StreamWriter textwriter = new StreamWriter(SubFolder + fileName);
            var csvout = new CsvWriter(textwriter);
            csvout.WriteHeader<DMT_INV_Part_Combined>();
            csvout.Flush();
            csvout.NextRecord();
            csvout.WriteRecords<DMT_INV_Part_Combined>(data);
            csvout.Dispose();
            textwriter.Close();

            var fi = new FileInfo(Path.Combine(SubFolder, fileName));
            var tmpFile = FixOutputHeader(fi.FullName);
        }

        private static void WritePartRevisions(string fileName, List<PartRevision> data)
        {
            StreamWriter textwriter = new StreamWriter(SubFolder + fileName);
            var csvout = new CsvWriter(textwriter);
            csvout.WriteHeader<PartRevision>();
            csvout.Flush();
            csvout.NextRecord();
            csvout.WriteRecords<PartRevision>(data);
            csvout.Dispose();
            textwriter.Close();
        }
